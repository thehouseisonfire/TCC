...
def do_test(proto_ver):
    ...
    connect_packet = mosq_test.gen_connect(
        "bridge-reconnect-test", keepalive=keepalive, proto_ver=proto_ver_connect
    )
    ...
    publish_packet = mosq_test.gen_publish(
        "bridge/reconnect",
        qos=0,
        payload="bridge-reconnect-message",
        proto_ver=proto_ver,
    )
    ...
    local_broker = mosq_test.start_broker(
        cmd=local_cmd,
        filename=os.path.basename(__file__) + "_local1",
        use_conf=False,
        port=port2,
    )
    ...
    if os.environ.get("MOSQ_USE_VALGRIND") is not None:
        ...
        time.sleep(0.5)
    local_broker = mosq_test.start_broker(
        cmd=local_cmd, filename=os.path.basename(__file__) + "_local2", port=port2
    )
    if os.environ.get("MOSQ_USE_VALGRIND") is not None:
        time.sleep(5)
    else:
    ...
    try:
        ...
        helper_connect_packet = mosq_test.gen_connect(
            "test-helper", keepalive=keepalive, proto_ver=proto_ver
        )
        helper_connack_packet = mosq_test.gen_connack(rc=0, proto_ver=proto_ver)
        helper_publish_packet = mosq_test.gen_publish(
            "bridge/reconnect",
            qos=1,
            mid=1,
            payload="bridge-reconnect-message",
        ...
        helper_sock = mosq_test.do_client_connect(
            helper_connect_packet,
            helper_connack_packet,
            port=port2,
            connack_error="helper connack",
        )
        mosq_test.do_send_receive(
            helper_sock, helper_publish_packet, helper_puback_packet, "puback"
        )
