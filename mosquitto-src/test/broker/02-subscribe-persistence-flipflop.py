...
def do_test(proto_ver):
    rc = 1
    keepalive = 60
    connect_packet_sub_persistent = mosq_test.gen_connect(
        "flipflop-test", keepalive=keepalive, clean_session=False, proto_ver=proto_ver
    )
    connect_packet_sub_clean = mosq_test.gen_connect(
        "flipflop-test", keepalive=keepalive, clean_session=True, proto_ver=proto_ver
    )
    connack_packet_sub = mosq_test.gen_connack(rc=0, proto_ver=proto_ver)

    connect_packet_pub = mosq_test.gen_connect(
        "flipflop-test-pub", keepalive=keepalive, proto_ver=proto_ver
    )
    ...
    publish_packet = mosq_test.gen_publish(
        "flipflop/test", qos=1, mid=mid, payload="message", proto_ver=proto_ver
    )
    ...
    try:
        ...
        sub_sock = mosq_test.do_client_connect(
            connect_packet_sub_persistent, connack_packet_sub, port=port
        )
        mosq_test.do_send_receive(
            sub_sock, subscribe_packet, suback_packet, "subscribe persistent 1"
        )
        # And disconnect
        sub_sock.close()
        ...
        sub_sock = mosq_test.do_client_connect(
            connect_packet_sub_clean, connack_packet_sub, port=port
        )
        ...
        mosq_test.do_send_receive(
            sub_sock, subscribe_packet, suback_packet, "subscribe persistent 2"
        )
