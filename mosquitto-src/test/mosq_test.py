...
def do_client_connect(
    connect_packet,
    connack_packet,
    hostname="localhost",
    port=1888,
    timeout=10,
    connack_error="connack",
):
...
def to_string(packet):
    ...
    if cmd == 0x00:
        ...
        if len(packet) == 5:
            ...
            return (
                "PUBACK, rl=" + str(rl) + ", mid=" + str(mid) + ", reason_code=" + str(reason_code)
            )
        else:
        ...
        if len(packet) == 5:
            (cmd, rl, mid, reason_code) = struct.unpack("!BBHB", packet)
            return (
                "PUBREC, rl=" + str(rl) + ", mid=" + str(mid) + ", reason_code=" + str(reason_code)
            )
        else:
            (cmd, rl, mid) = struct.unpack("!BBH", packet)
...
def gen_connect(
    ...
    if will_topic != None:
        ...
        if len(will_payload) > 0:
            packet = packet + struct.pack(
                "!H" + str(len(will_payload)) + "s", len(will_payload), will_payload
            )
        else:
            packet = packet + struct.pack("!H", 0)
