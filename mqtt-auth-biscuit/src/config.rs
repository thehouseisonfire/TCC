...
pub fn parse_options(options: *mut crate::MosquittoOpt, option_count: i32) -> Result<PluginConfig, String> {
    ...
    for i in 0..option_count {
        ...
        let Some((key, value)) = opt_kv(opt_ptr) else { continue };
    ...
    let decoding_key = match alg {
        Algorithm::HS256 => {
            let secret = jwt_hmac_secret.ok_or_else(|| "jwt_hmac_secret is required for HS256".to_string())?;
            DecodingKey::from_secret(secret.as_bytes())
        }
        Algorithm::RS256 => {
            let path = jwt_key_file.ok_or_else(|| "jwt_key_file is required for RS256".to_string())?;
            let pem = fs::read(path).map_err(|e| format!("Failed reading jwt_key_file: {e}"))?;
            DecodingKey::from_rsa_pem(&pem).map_err(|e| format!("Invalid RSA public key PEM: {e}"))?
        }
        Algorithm::ES256 => {
            let path = jwt_key_file.ok_or_else(|| "jwt_key_file is required for ES256".to_string())?;
            let pem = fs::read(path).map_err(|e| format!("Failed reading jwt_key_file: {e}"))?;
    ...
    let biscuit_root_public_key = match (biscuit_root_key_hex, biscuit_root_private_key_hex) {
        (Some(pub_hex), _) => {
            let bytes = hex::decode(pub_hex).map_err(|e| format!("Invalid biscuit_root_key_hex: {e}"))?;
            if bytes.len() != 32 {
        ...
        (None, Some(priv_hex)) => {
            let bytes = hex::decode(priv_hex).map_err(|e| format!("Invalid biscuit_root_private_key_hex: {e}"))?;
            if bytes.len() != 32 {
                return Err("biscuit_root_private_key_hex must decode to exactly 32 bytes".to_string());
            }
        ...
        (None, None) => {
            return Err("biscuit_root_key_hex or biscuit_root_private_key_hex is required".to_string());
        }
    };
