...
The \texttt{mosquitto-go-auth} is an authentication and authorization module for Mosquitto that stood out for supporting various backends, including Redis, MongoDB, and JWT, and for using the Go language via FFI. However, the repository was archived in June 2025 and does not offer native support for Biscuit, leaving a gap for more expressive capability tokens. The \texttt{mosquitto-auth-plug}, the oldest solution, archived in 2019, also does not contemplate this functionality. In addition to these community modules, Mosquitto's official \emph{Dynamic Security} module equips the \emph{broker} with dynamic ACLs but does not expand the authentication and authorization mechanisms for the use of tokens \cite{mosquitto_go_auth,mosquitto_auth_plug,eclipse_mosquitto_dynamic}. The absence of active implementations that integrate Biscuit into Mosquitto reinforces the practical relevance of this work.

Works such as \citeonline{philipp2023_daxiot} propose decentralized authentication and authorization schemes for IoT networks, demonstrating how Decentralized Identifiers and Verifiable Credentials (VCs) can be applied to MQTT. This approach indicates the viability of paradigms where Biscuit presents architectural advantages, due to its native support for privilege attenuation and offline delegation, features not directly explored in the work of \citeonline{philipp2023_daxiot}.

\citeonline{anantharaman2019} demonstrate the use of Macaroons in \emph{publish-subscribe} architectures, highlighting gains in offline delegation and pointing out challenges in distributed key management. Although Macaroons share with Biscuit the capability of permission attenuation, the work does not quantify the computational impact of these operations on \emph{brokers} with resource constraints. \citeonline{cochak2024} discuss a nested token model and signature aggregation techniques to reduce verification cost, offering useful architectural alternatives to evaluate Biscuit. However, the practical application of these techniques in the MQTT context remains unexplored in the literature.

\citeonline{fossen2022exploring} mentions, in its exploration of capability-based security in Rust, that Biscuit has the potential to address limitations in access control implementations, highlighting its application as future work. \citeonline{identity_management_for_agents} identify, in their study on identity management and AI agent security, the need to attenuate permissions so that these agents can safely delegate tasks to specialized sub-agents, citing Biscuit and Macaroons as viable solutions. Both works underscore the theoretical relevance of Biscuit but do not provide empirical data on its viability in MQTT environments.

A notable production system is the deployment of Apache Pulsar by Clever Cloud, a \emph{publish-subscribe} platform, in which the company uses Biscuit and makes its integration module available, demonstrating the practical applicability of the format in distributed messaging systems \cite{clever_cloud_pulsar}. However, Pulsar's architecture, focused on high computational capacity and persistent storage, differs significantly from Mosquitto's, a \emph{broker} designed for lightness and operation on resource-constrained devices, which makes a specific evaluation for the MQTT ecosystem necessary.

Despite these advances, there are currently no systematic comparisons between Biscuit and JWT in the MQTT ecosystem, nor studies that quantify the technical viability of Biscuit in the authentication and authorization operations of a lightweight \emph{broker} such as Mosquitto. Furthermore, the archiving of the main community modules opens space for a modern solution that incorporates Biscuit and enables controlled experiments with performance metrics, resource consumption, and scalability. This work fills this gap by providing both a functional implementation and an empirical analysis of the architectural implications of adopting this new capability token in IoT infrastructures.
