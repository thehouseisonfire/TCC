...
def helper(port, proto_ver):
    ...
    publish_packet = mosq_test.gen_publish(
        "test", qos=0, payload="mount point", proto_ver=proto_ver
    )

    sock = mosq_test.do_client_connect(
        connect_packet, connack_packet, port=port, connack_error="helper connack"
    )
...
def do_test(proto_ver):
    ...
    publish_packet1 = mosq_test.gen_publish(
        "mount/test", qos=0, payload="mount point", proto_ver=proto_ver
    )
    ...
    try:
        sock1 = mosq_test.do_client_connect(
            connect_packet1, connack_packet1, timeout=20, port=port1
        )
        mosq_test.do_send_receive(sock1, subscribe_packet1, suback_packet1, "suback1")

        sock2 = mosq_test.do_client_connect(
            connect_packet2, connack_packet2, timeout=20, port=port2
        )
