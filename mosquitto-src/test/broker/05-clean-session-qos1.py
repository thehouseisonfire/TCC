...
def helper(port):
    ...
    publish_packet = mosq_test.gen_publish(
        "qos1/clean_session/test", qos=1, mid=mid, payload="clean-session-message"
    )
...
def do_test(proto_ver):
    ...
    subscribe_packet = mosq_test.gen_subscribe(
        mid, "qos1/clean_session/test", 1, proto_ver=proto_ver
    )
    suback_packet = mosq_test.gen_suback(mid, 1, proto_ver=proto_ver)

    mid = 1
    publish_packet = mosq_test.gen_publish(
        "qos1/clean_session/test",
        qos=1,
        mid=mid,
        payload="clean-session-message",
        proto_ver=proto_ver,
    )
